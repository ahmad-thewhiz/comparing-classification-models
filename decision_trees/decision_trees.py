...
def main(dataset_dir, output_dir):
    ...
    start = time()
    gridSearch.fit(X_train_tf, trainY)
    end = time()
    ...
    best_dt_classifier = gridSearch.best_estimator_
    start = time()
    y_pred = best_dt_classifier.predict(X_test_tf)
    y_pred_proba = best_dt_classifier.predict_proba(X_test_tf)[:, 1]  # Probability estimates for the positive class
    ...
    with open(f'{output_dir}/best_dt_classifier_model.pkl', 'wb') as model_file:
        pickle.dump(best_dt_classifier, model_file)
    with open(f'{output_dir}/tfidf_vectorizer.pkl', 'wb') as vec_file:
        pickle.dump(tf_vec, vec_file)

    print("Model saved as 'best_dt_classifier_model.pkl'")
